...
model <- lm(pobreza_general ~ participacion_ciudadana, data = df_merge)
...
```{r}
ggplot(df_merge, aes(x = participacion_ciudadana, y = pobreza_general)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE, color = "blue") +
  labs(title = "Pobreza y participación ciudadana",
...
Educacion
```{r}
# Assuming svymean output is saved in a variable
education_proportions <- svymean(~educacion, design = survey_design_adultos)

# Convert to a data frame if necessary
education_proportions_df <- as.data.frame(education_proportions)

# Add a cumulative sum column
education_proportions_df$cumulative_sum <- cumsum(education_proportions_df$mean)

# View the result
education_proportions_df
