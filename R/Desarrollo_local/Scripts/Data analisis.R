...
ggplot(Datos_fundesa, aes(x=WES_SCORE_mean,y=Ladinos_porcentaje)) + 
  geom_point()
geom_label(
...
ggplot(Datos_fundesa, aes(x=Ladinos_porcentaje,y=WES_SCORE_mean)) + 
  geom_point()

LR=lm(WES_SCORE_mean ~ Ladinos_porcentaje, data =Datos_fundesa)
summary(LR)
...
match_obj <- matchit(Turistico ~ AREA_mean +Densidad.poblacional+ AGRICULTURA  + Ladinos_porcentaje,
                     data = Datos_fundesa[-c(1),], method = "nearest", distance ="glm",#Se quitó Guatemala del Matching
                     ratio = 1,
                     replace = FALSE)
summary(match_obj)
...
res <- lm(Tasa.global.de.participacion.indigena ~ Turistico, data = matched_data, weights = weights)
#Test the coefficient using cluster robust standard error
coeftest(res, vcov. = vcovCL, cluster = ~subclass)
#Calculate the confidence intervals based on cluster robust standard error
coefci(res, vcov. = vcovCL, cluster = ~subclass, level = 0.95)

res <- lm(Tasa.global.de.participacion.mujeres ~ Turistico, data = matched_data, weights = weights)
#Test the coefficient using cluster robust standard error
coeftest(res, vcov. = vcovCL, cluster = ~subclass)
#Calculate the confidence intervals based on cluster robust standard error
coefci(res, vcov. = vcovCL, cluster = ~subclass, level = 0.95)
...
Datos_fundesa$Indicador.502..Ingresos.propios.por.habitante..sin.regalías.

res <- lm(Indicador.502..Ingresos.propios.por.habitante..sin.regalías. ~ Turistico, data = matched_data, weights = weights)
#Test the coefficient using cluster robust standard error
coeftest(res, vcov. = vcovCL, cluster = ~subclass)
#Calculate the confidence intervals based on cluster robust standard error
coefci(res, vcov. = vcovCL, cluster = ~subclass, level = 0.95)
